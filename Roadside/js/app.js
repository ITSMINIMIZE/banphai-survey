...
const App = {
          ...
          DataRound.load(FB.db).then(() => {
            ...
            return WipeCommand.check(FB.db, DB, r => {
              const msg = r.mode === 'all'
                ...
                : `ล้างข้อมูลรอบก่อนออกจากเครื่องแล้ว ${r.removed} จุดสำรวจ`;
  ...
  SS_SESSION: '_ri_session_v1',
  SS_VIEW:    '_ri_view_v1',
  LS_NAME:    '_ri_last_surveyor_v1',
  ...
  _saveView() { this._jset(sessionStorage, this.SS_VIEW, { page: this.page, stId: this.stId, ivId: this.ivId }); },
  ...
  _restoreView() {
    ...
    try {
    if ((v.page === 'station' || v.page === 'wizard') && DB.getStation(v.stId)) { this.navigate('station', v.stId); return true; }
    if (v.page === 'interview' && DB.getStation(v.stId)) { this.navigate('interview', v.stId, v.ivId); return true; }
  ...
  loginAsAdmin() {
    this.showModal('🔐 เข้าสู่ระบบ (ผู้ดูแล / ผู้ควบคุม)', `
      <div class="form-row">
        ...
        <div style="font-size:11px;color:var(--gray-500);margin-top:3px;">ใช้อีเมลที่ผู้ดูแลระบบลงทะเบียนให้ — ทั้งผู้ดูแลและผู้ควบคุม</div>
      </div>
      <div class="form-row">
