...
const App = {
    ...
    return `<div class="page container">
      ...
      <div class="sec-header">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          ...
          <button class="btn btn-ghost btn-sm" onclick="App.generateMockData()" title="สร้างข้อมูลจำลอง 100 ครัวเรือน (Dev)">🎲 ข้อมูลจำลอง</button>
  ...
  // ===================== DEV: MOCK DATA =====================
  generateMockData() {
    if (!confirm('สร้างข้อมูลจำลอง 100 ครัวเรือน?\n(ข้อมูลจำลองเก่าจะถูกแทนที่ ข้อมูลจริงไม่หาย)')) return;
    ...
    const FARE_M = ['รถจักรยานยนต์รับจ้าง','รถสองแถว / รถประจำทาง'];
    const DESTS  = [
    ...
    const makeTrips = (homeAddr, homeCoord) => {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const dBase = isLast
          ...
          : R(DESTS);
        const d = { ...dBase, c: dBase.c || coord(0.02) };
        const mode = R(MODES);
        const dur  = RI(5, 40);
        const fare = FARE_M.includes(mode) ? String(R([20,30,40,50,60])) : '';
    ...
    const makeHH = (i) => {
      ...
      const vehicles = hasV === 'มี' ? { motorcycle: { private: RI(1,2), company:0, gov:0 } } : {};
      return {
    ...
    let db = { households: [] };
    try { db = JSON.parse(localStorage.getItem(DB.KEY) || '{"households":[]}'); } catch {}
