...
const Issues = {
  ...
  member(m) {
    ...
    if (trips.length === 1) {
      ...
      const seq = trips.slice().sort((a, b) => (a.seq || 0) - (b.seq || 0));
      if (seq[0].purpose === 'กลับบ้าน')
        hard.push('เที่ยวแรกเป็น "กลับบ้าน" ไม่ได้ — ต้องออกจากบ้านก่อน');
      // กลับถึงบ้านแล้วต้องออกจากบ้านก่อน ถึงจะกลับบ้านได้อีก
      for (let i = 1; i < seq.length; i++) {
        if (seq[i].purpose === 'กลับบ้าน' && seq[i - 1].purpose === 'กลับบ้าน') {
          hard.push(`เที่ยวที่ ${seq[i].seq || i + 1} เป็น "กลับบ้าน" ซ้อนกับเที่ยวก่อนหน้า — ต้องออกจากบ้านก่อน`);
          break;
        }
      }
