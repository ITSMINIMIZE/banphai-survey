...
const Issues = {
  ...
  member(m) {
    ...
    if (trips.length === 1) {
      ...
      // ต้องมีเวลาครบทุกเที่ยวก่อนถึงจะตัดสินได้ว่าเที่ยวไหนมาก่อน
      // ถ้าเที่ยวขาไปยังไม่ได้กรอกเวลา มันจะหายไปจากการเรียงลำดับ แล้วขากลับที่มีเวลา
      // จะกลายเป็น "เที่ยวแรก" ทั้งที่ไม่ใช่ → ขึ้นเตือนผิดคน
      // (เที่ยวที่ไม่มีเวลามีกฎ 'ไม่ระบุเวลาออกเดินทาง' จับอยู่แล้ว กรอกครบเมื่อไหร่กฎนี้ค่อยทำงาน)
      if (trips.every(t => t.departureTime)) {
        const first = trips.reduce((a, t) => String(t.departureTime) < String(a.departureTime) ? t : a);
