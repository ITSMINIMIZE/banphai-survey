...
const Issues = {
  household(hh) {
    ...
    // เดิมมีกฎ 'ไม่มีสมาชิกคนไหนเดินทางเลย' ตรงนี้ด้วย — ถอดออกแล้ว
    // เพราะกฎระดับคนจับได้ครบทุกคนอยู่แล้ว (รวมกรณีที่ไม่มีใครเดินทางเลย)
    // ถ้าเก็บไว้ ปัญหาเดียวจะถูกนับสองครั้ง แล้วเลข "ต้องแก้ N" บนการ์ดจะเฟ้อ
    // และกฎเดิมยังชี้ไม่ได้ด้วยว่าเป็นคนไหน
  ...
  member(m) {
    ...
    const trips = m.trips || [];
    // ⚠️ ต้องมีสาขาของ "0 เที่ยว" ด้วย — ของเดิมเป็น if/else if ที่เริ่มนับจาก 1
    //    สมาชิกที่ยังไม่ได้กรอกการเดินทางเลยจึงหลุดทุกกฎ ไม่มีอะไรเตือนสักตัว
    //    และกฎระดับบ้านก็ใช้ .some() (มีอย่างน้อยหนึ่งคนเดินทางก็ผ่าน) จึงเงียบตามกันไป
    //    ฟอร์มไม่มีช่องบอกว่า "ไม่ได้เดินทาง" และปุ่มเสร็จสิ้นบังคับ 2 เที่ยวอยู่แล้ว
    //    → 0 เที่ยว = เก็บข้อมูลไม่ครบเสมอ ไม่ใช่คนที่อยู่บ้านทั้งวัน
    if (trips.length === 0) {
      hard.push('ยังไม่ได้กรอกการเดินทางเลย');
